...
class General(Plugin):
    def __init__(self):
        self.bot: Gojo
        super().__init__(name="general", description="General bot commands.", default_enabled_guilds=(964157647752400916) )
...
async def av_command(context: SlashContext      ) -> None:
    ...
    await context.respond(
        ...
        ).set_author(name=f"{target.username.replace('_','_/')}'s avatar").set_footer(text=f"Requested by {context.author}").set_image(target.display_avatar_url)
    )
