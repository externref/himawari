...
class Gojo(lightbulb.BotApp):
    ...
    def __init__(self):
        self.color_handler = ColorHandler()         # color handler to deal with bot's custom embed colors.
        self.goodbye_handler = GoodbyeHandler()     # goodbye database connection for MemberDeleteEvent events.
        self.welcome_handler = WelcomerHandler()    # welcomer database connection for greeting new users.
        self.starboard_handler = StarboardHandler() # starboard manager with required methods.
        super().__init__(
            token=get_token(),
        ...
        self.load_extensions_from("src/extensions") # loading all plugins and help command from the extensions folder.
        self.invite_url = (
